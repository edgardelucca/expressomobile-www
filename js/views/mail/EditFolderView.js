define([
	...
	render: function() {
		...
		foldersCol.getFolders(this.folderID, this.search).done(function(foldersData) {
		    ...
		    that.globalName = currentFolder.get("globalName");
		    that.renderView();
	...
	renderView: function() {
		...
		var parentGlobal =  Shared.folders.getFolderByID(this.parentFolderID);
		this.globalName = parentGlobal.get('globalName');
	    }
	    if (this.action == "renameFolder") {
		title = "Renomear Pasta";
	    }
	...
	cleanTrash: function(folderID) {
	    var foldersCol = new FoldersCollection();
	    foldersCol.cleanTrash(folderID).done(function(result) {
		var message = {
		    type: "success",
		    icon: 'icon-email',
	...
	saveFolder: function() {
		...
		foldersCol.addFolder(folderName, parentFolderID, globalName).done(function(result) {
		    var message = {
			type: "success",
			icon: 'icon-email',
		...
		foldersCol.renameFolder(folderName, folderID, globalName).done(function(result) {
		    var message = {
			type: "success",
			icon: 'icon-email',
	...
	deleteFolder: function(folderID) {
	    ...
	    var globalName = currentFolder.get("globalName");

	    foldersCol.deleteFolder(folderID, globalName).done(function(result) {
		var message = {
		    type: "success",
		    icon: 'icon-email',
